...
generate_incidence_matrix <- function(
  ...
  topics_per_agent[affiliation_sample] <- affiliation_total -
    sum(topics_per_agent[-affiliation_sample])
...
generate_adjacency_matrix <- function(
  n_agents,
  avg_degree,
  lognormal_mu,
  lognormal_sigma,
  ...
  fitness_scores <- rlnorm(
    n_agents,
    meanlog = lognormal_mu,
    sdlog = lognormal_sigma
  )
...
run_diffusion <- function(
  ...
  repeat {
    ...
    active_influence_mat@x <- influence_mat@x *
      current_adoption[influence_mat@i + 1]

    active_neighbor_influence <- network_mat %*% active_influence_mat
    active_neighbor_influence_sum <- rowSums(
      active_neighbor_influence * affiliation_mat
    )

    active_neighbor_count <- as.numeric(network_mat %*% current_adoption)
    active_neighbor_count[active_neighbor_count == 0] <- 1

    active_neighbor_influence_rescale <- active_neighbor_influence_sum /
    ...
    if (
      all(current_adoption == previous_adoption) ||
  ...
  if (adopters_count > 1) {
    ...
    realized_density <- length(sub_mat@i) /
      (adopters_count * (adopters_count - 1))
  }
